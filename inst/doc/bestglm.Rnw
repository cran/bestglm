...
\author{A. I. McLeod\\University of Western Ontario \And
        Changiang Xu\\University of Western Ontario}
\Plainauthor{A. I. McLeod, Changjiang Xu}
...
\Abstract{
Functions are provided for selecting the best subset of inputs for
the \GLM~ family.
The criterion available include a variety of information criteria as well as cross-validation.
...
We use the lexicographically method suggested by \citet{Morgan72} for the all subsets regression
...
option is used.
For compatibility with the \pkg{leaps} package, \code{Xy} may also
be an \code{lm} or \code{biglm} object.
...
Normally the information criteria will eliminate unnecessary variables automatically and so
...
The information criterion include the usual AIC and BIC as well as two types of 
...
The information criterion we consider are all based on a penalized form of the deviance
...
Many other criterion which are essentially equivalent to the \AIC\  have also been suggested.
Several other asymptotically equivalent but more specialized criterion were suggested
...
the AIC.
In practice, with small $n$, these criterion often select the same model.
...
\subsubsection[Theorem for q-Interval Calculation]{Theorem for q-Interval Calculation}
\label{subsubsec:TheoremQ}

Consider the generalized information criterion \citep{Akaike79},
\begin{equation}\label{Clmd}
\textsc{c}_{\lambda}(k)=-2 \log L(\hat\theta_k) + \lambda k,
\end{equation}
where $-\infty < \lambda <\infty$ is an adaptive penalty.
The criterion is called \AICz-type if $\lambda$ is a constant
and \BICz-type if $\lambda$ depends on $n$.
The consistency property of \BIC is shared by any \BICz-type criterion that has a penalty of the form $\lambda k$
satisfying $\lambda \rightarrow\infty$ and $\lambda/n \rightarrow 0$ as $n\rightarrow \infty$~\citep{Shao97, Yang05}.
For each $q$, \QBIC is the same as \Clmd with $\lambda=\log n-2\log q/(1-q)$.
Conversely, for each $\lambda$, there exists a
$q = ( 1 + n^{-0.5} e^{0.5\lambda} )^{-1}$
such that \QBIC is the same as \Clmdz.
Hence \QBIC is equivalent to \Clmdz.

As $\lambda$ varies between $-\infty$ and $+\infty$, $\argmin_k \textsc{c}_{\lambda}(k)$ varies between
$0$ and $K$.
Denote these values by $k_0 < \ldots < k_{\cal K}$, where ${\cal K} \le K$ is the number
of different model sizes that can be selected, $k_0 = 0$ and $k_{\cal K} = K$.
Each of these possible models sizes, $k_i, i=0,\ldots, \cal K$ corresponds to a range
$\lambda$ values, $d_{1,k_i} < \lambda < d_{2, k_i}$.
The following Lemma gives explicit formula for determining these ranges.
For simplicity, let ${\cal L}_k = \log L(\hat\theta_k)$.

\begin{lemma}\label{LemmaClmd}
Set
$$
\begin{array}{rl}
d_{1,k} &= \min_{j<k} 2({\cal L}_j - {\cal L}_k)/(j-k), k=1,\ldots, K;\\
d_{2,k} &= \max_{j>k} 2({\cal L}_j - {\cal L}_k)/(j-k), k=0,\ldots,K-1;
\end{array}
$$
$d_{2,K}=-\infty$
and
$d_{1,0}=\infty$.
The model of size $k$ is selected using the criterion \Clmd
if $d_{2,k} < \lambda < d_{1,k}$.
If $\lambda = d_{i,k}$, there is a tie between adjacent model sizes.
\end{lemma}

By setting $\lambda=\log n-2\log q/(1-q)$,
the following theorem can be directly derived from Lemma~\ref{LemmaClmd}.

\begin{theorem}\label{Theoremq}
The \QBIC\ can select the model size $k$ if and only if $q_{1,k} \le q \le q_{2,k}$,
where $q_{i,k} = ( 1 + n^{-0.5} e^{0.5 d_{i,k}} )^{-1}$,
$k = 0, \dots, K$.
When $q_{1,k} > q_{2,k}$ the model of size $k$ cannot be selected.
\end{theorem}

From Theorem~\ref{Theoremq}, the best choice of $q$ is not unique and corresponds to an interval.
Assuming the true model is of size $k_0$ then
the best choice is $q\in (q_{1,k_0}, q_{2,k_0})$.
For $k>k_0$, $2({\cal L}_k - {\cal L}_{k-1}) \sim \chi^2_1$ asymptotically
and hence $\pr(d_{1,k} \le \xi) \ge \alpha$,
where $\xi$ is the $\alpha$ quantile of $\chi_1^2$,
since $d_{1,k} \le 2({\cal L}_k - {\cal L}_{k-1})$.
Conversely, if $d_{1,k}>\xi$
then $k>k_0$ would be not true with confidence probability $\alpha$.
Let $d_{1,k_1}$ be the smallest $d_{1,k}$ subject to $d_{1,k}>\xi$.
The corresponding value of $q$, $q_{1,k_1}$, is obtained by Theorem~\ref{Theoremq}.
%$q_{1,k_1} = ( 1 + n^{-0.5} e^{0.5d_{1,k_1}} )^{-1}$.
The $q_{1,k_1}$ is a lower bound for $q$
because it corresponds to $d_{1,k_1}>\xi$
and so $k_1\le k_0$ with confidence probability greater than or equal to $\alpha$.

Notice that $2({\cal L}_j - {\cal L}_k)  \sim \chi^2_{j-k}$ asymptotically for $j > k \geq k_0$,
and
$2({\cal L}_j - {\cal L}_k) \sim \chi^2_{j-k}(\delta)$ asymptotically for $j>k_0>k$,
where $\delta>0$ is a non-centrality parameter.
Hence as $k$ decreases from $K$ to $k_0$,
the difference, $d_{1,k}-d_{2,k}$, has a jump increase at $k_0$.
Let $d_{2,k_2}$ be the value $d_{2,k}$ for which $d_{1,k}-d_{2,k}$ is largest
subject to $d_{1,k}\le\xi$,
and denote the corresponding value of $q$ by $q_{2,k_2}$.
Since $d_{2,k_2}<d_{1,k_2}\le \xi <d_{1,k_1}$,
$q_{1,k_1}<q_{2,k_2}$,
$(q_{1,k_1}, q_{2,k_2})$ provides an interval for choosing $q$,
which covers $(q_{1,k_1}, q_{2,k_1})$ and $(q_{1,k_2}, q_{2,k_2})$.
When $\xi\le\delta$,
$(q_{1,k_0}, q_{2,k_0}) \subseteq (q_{1,k_1}, q_{2,k_2})$
and $q_{2,k_2}=q_{2,k_0}$ asymptotically
with confidence probability greater than or equal to $\alpha$.
Often we take $\alpha=0.99$ and then $\xi = 6.63$.

\subsubsection[Numerical Illustration for Theorem]{Numerical Illustration for Theorem}
...
\cite{Breiman1984} originally suggesting rule for selecting the best prunned CART.
...
bestglm(Xy, IC="CV", CVArgs=list(Method="DH", K=10, REP=1))  
@
After 1000 simulations, the results are summarized below.
\begin{tabular}{lllllllll}
Number of inputs selected &  1 &  2 &  3  & 4 &  5 &  6 &  7  & 8 \\
...
\subsection{South Afrian Heart Disease}
...
in \citet{XuMcLeod2009, Xu2009} was to compare different information
